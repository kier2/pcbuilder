...
const currentParts = computed(() => {
  ...
  const partsList = allPeripheralData.value[props.selectedItem.slug] || [];
  return partsList;
});
...
const handleSelectedParts = (id, name, img, priceUsd, pricePhp ) => {
  const selectedPart = allPeripheralData.value[props.selectedItem.slug]?.find(p => p.id === id);
...
<template>
  <div class="h-screen overflow-y-auto overflow-x-hidden shadow sm:rounded-md w-full md:h-[75%] md:sticky md:top-0 custom-scrollbar">
    ...
    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <h2 class="text-white text-center">Select Peripheral First</h2>
    </div>
